...
then
    ...
    export SEDCMD="s#console=ttyS1,115200n8#$CMDLINE#g"
    echo $SEDCMD > $TMPDIR/sedcommand
    cp $TMPDIR/$BOOTIMAGEFILE-Image $DEVICE_DIR/kernel
    popd > /dev/null
else
    mkdir -p $DEVICE_DIR
...
do
    ...
    cat $file | sed s/j1minive3gxtc/$DEVICE/g | sed s/samsung/$MANUFACTURER/g | sed -f $TMPDIR/sedcommand | sed s/0x00000000/$BASE/g | sed s/2048/$PAGESIZE/g > $OUTPUT_FILE
done
